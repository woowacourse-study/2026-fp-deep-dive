...
function createMemberObject(id, name, points) {
    return {
        id: id,
        name: name,
        grade: "normal",
...
function getMemberGrade(currentMember) {
    if (currentMember.totalUsageHours >= gradeConfig.master.minHours) {
        return "master";
    }
    else if (currentMember.totalUsageHours >= gradeConfig.honor.minHours) {
        return "honor";
    }
    else {
        return "normal";
...
function updateCurrentMember(point, totalUsageHours) {
    currentMember.points += point;
    currentMember.totalUsageHours += totalUsageHours;
    currentMember.grade = getMemberGrade(currentMember);
}

function registerMember(id, name, points) {
    currentMember = createMemberObject(id, name, points);
    reservations = [];
...
function validateParticepantLimit(attendees, room) {
    if (attendees > room.capacity) {
        throw new Error("인원이 초과되었습니다. 최대 수용 인원: " + room.capacity + "명");
    }
}

function makeReservationObject(currentMember, room, date, startHour, duration, attendees) {
    var fee = calculateFee(room.pricePerHour, duration);
    var pointRate = calculatePointRate(currentMember.grade)
    var earnedPoints = calculateEarnedPoints(fee, pointRate);
...
function printReservationResult(roomName, date, startHour, duration, earnedPoints, CrurrentMemberPoint) {
    console.log("예약 완료! [" + roomName + "] " + date + " " + startHour + "시 (" + duration + "시간)");
    console.log("적립 포인트: +" + earnedPoints + "P  |  보유 포인트: " + CrurrentMemberPoint + "P");
}
...
function makeReservation(roomId, date, startHour, duration, attendees) {
    ...
    validateParticepantLimit(attendees, room);

    // 게시물 등록 API와 같이 ID를 제외한 완성된 객체를 받고 등록 시점에 ID를 달아줌, makeReservationObject을 테스트하기 좋게 만들어준다.
    var reservation = {
        ...makeReservationObject(currentMember, room, date, startHour, duration, attendees),
        id: "RES-" + Math.random()
    }

    updateCurrentMember(reservation.earnedPoints, reservation.duration);

    reservations.push(reservation);

    printReservationResult(room.name, date, startHour, duration, reservation.earnedPoints, currentMember.points);
...
function validateNotCancellableReservation(reservation) {
    if (!reservation || reservation.status === "cancelled") {
        throw new Error("취소할 수 없는 예약입니다.");
    }
}

function printCancelReservationResult(currentMember, reservation, penalty) {
    console.log("예약이 취소되었습니다: " + reservation.id);
...
function cancelReservation(reservationId, hoursUntilStart) {
    ...
    validateNotCancellableReservation(reservation);

    var penaltyRate = getPenaltyRate(currentMember.grade);
    var penalty = calculatePenalty(penaltyRate, reservation, hoursUntilStart);

    updateCurrentMember(-(reservation.earnedPoints + penalty), -reservation.duration);

    reservation.status = "cancelled";

    printCancelReservationResult(currentMember, reservation, penalty);
}
